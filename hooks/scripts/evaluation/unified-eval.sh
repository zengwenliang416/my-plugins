...
# === 主输出 ===
{
    ...
    echo "2. **调用格式**：\`Skill(\"agent-name\")\` 或 \`Skill(\"domain:skill-name\")\`"
    ...
    echo ""
    echo "**违规示例**："
    echo "- ❌ 用户说\"提交代码\" → 直接 \`git commit\`（绕过 commit-orchestrator）"
    echo "- ❌ 用户说\"修复 Bug\" → 直接编辑代码（绕过 debug-orchestrator）"
    echo "- ❌ 用户说\"看看代码有什么问题\" → 直接 Read 文件（绕过 review-orchestrator）"
    echo "- ❌ 用户说\"分析这个模块优化点\" → 直接 Grep 搜索（绕过 dev-orchestrator）"
    echo ""
    echo "**正确示例**："
    echo "- ✅ 用户说\"提交代码\" → \`Skill(\"commit-orchestrator\")\`"
    echo "- ✅ 用户说\"修复 Bug\" → \`Skill(\"debug-orchestrator\")\`"
    echo "- ✅ 用户说\"审查代码质量\" → \`Skill(\"review-orchestrator\")\` 或 \`Skill(\"ccg:review\")\`"
    echo "- ✅ 用户说\"看看这个文件需要优化\" → \`Skill(\"review-orchestrator\")\`"
