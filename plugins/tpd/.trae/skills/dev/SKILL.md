...
# /dev - OpenSpec Development Workflow Command
...
**Supports no-argument invocation**: When executing `/dev` directly, it will automatically read OpenSpec Active Change and let user confirm the proposal.
...
- ✅ Execute 调用 /openspec:archive after all tasks complete
...
0. **Step 0: OpenSpec Status Check**
   ...
   - If OpenSpec not initialized → prompt to execute `/init` first
   - Apply proposal:
     ```
     调用 /openspec:apply，参数：${PROPOSAL_ID}
     ```
...
1. **Step 1: Initialization & Previous Phase Integration**
   ...
   - **🔗 Plan Phase Integration** (MANDATORY):
     ...
     if [ ! -f "${PLAN_DIR}/plan.md" ]; then
       echo "❌ ERROR: Plan phase not found. Execute /plan first."
       exit 1
     fi
   ...
   - **⏸️ Hard Stop**: Display task scope with constraints and test requirements, ask user:
...
4. **Step 4: Refactor & Constraint-Aware Side-effect Review**
   - Execute refactor implementation with constraint awareness:
     ```
     调用 /code-implementer，参数：run_dir=${DEV_DIR} constraints_ref=plan-constraints.md pbt_ref=plan-pbt.md
     ```
...
6. **Step 6: Task Completion & Iteration**
   ...
   - **⏸️ Hard Stop**: Ask user whether to proceed to next phase:
...
When `TEAM_MODE=false` (default): Steps 3 and 5 execute as documented above with standard agent calls. **Zero behavior change** from baseline.
