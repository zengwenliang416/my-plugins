...
if [[ "$TASK_TYPE" == "fullstack" || "$TASK_TYPE" == "backend" ]]; then
  if [[ ! -f "${run_dir}/codex-plan.md" ]]; then
    echo "Error: Missing codex-plan.md, please execute /codex-planner first"
    exit 1
  fi
fi
...
if [[ "$TASK_TYPE" == "fullstack" || "$TASK_TYPE" == "frontend" ]]; then
  if [[ ! -f "${run_dir}/gemini-plan.md" ]]; then
    echo "Error: Missing gemini-plan.md, please execute /gemini-planner first"
    exit 1
  fi
fi
```
...
Read 工具读取 `${run_dir}/codex-plan.md` (if exists)
Read 工具读取 `${run_dir}/gemini-plan.md` (if exists)
...
使用 Edit 工具写入 `${run_dir}/architecture.md`:
...
Next step: Use /task-decomposer for task decomposition
