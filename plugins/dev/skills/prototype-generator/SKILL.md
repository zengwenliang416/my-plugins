...
| MCP 工具              | 用途                               | 触发条件        |
| --------------------- | ---------------------------------- | --------------- |
| `sequential-thinking` | 结构化原型生成，确保代码质量一致性 | 🚨 每次执行必用 |

## 执行流程

### Step 0: 结构化生成规划（sequential-thinking）

🚨 **必须首先使用 sequential-thinking 规划生成策略**

```
mcp__sequential-thinking__sequentialthinking({
  thought: "规划原型生成策略。需要：1) 理解分析方案 2) 确定代码结构 3) 识别依赖关系 4) 规划文件组织 5) 定义接口契约",
  thoughtNumber: 1,
  totalThoughts: 5,
...
1. **分析方案理解**：从 analysis-{model}.md 提取实施步骤
2. **代码结构确定**：确定模块/类/函数组织方式
3. **依赖关系识别**：识别内部模块和外部库依赖
4. **文件组织规划**：确定新增/修改文件列表
5. **接口契约定义**：定义模块间通信接口
