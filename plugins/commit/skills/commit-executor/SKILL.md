...
| MCP 工具              | 用途                                 | 触发条件        |
| --------------------- | ------------------------------------ | --------------- |
| `sequential-thinking` | 结构化提交执行策略，确保操作安全可靠 | 🚨 每次执行必用 |

## 执行流程

### Step 0: 结构化提交执行规划（sequential-thinking）

🚨 **必须首先使用 sequential-thinking 规划提交执行策略**

```
mcp__sequential-thinking__sequentialthinking({
  thought: "规划提交执行策略。需要：1) 读取提交信息 2) 解析选项 3) 验证暂存区 4) 构建并执行命令 5) 收集结果并写入",
  thoughtNumber: 1,
  totalThoughts: 5,
...
1. **提交信息读取**：从 commit-message.md 提取标题、正文、Footer
2. **选项解析**：处理 no_verify、amend、signoff、dry_run 选项
3. **暂存区验证**：检查是否有暂存变更（amend 模式除外）
4. **命令执行**：构建 HEREDOC 格式命令并执行 git commit
5. **结果收集**：获取提交哈希等信息，写入 commit-result.json
