...
description: |
  ...
  【不触发】没有提供图片的场景
  【🚨 强制】必须使用 gemini-cli skill 分析图片，不可跳过
  【依赖】gemini-cli（参考 skills/gemini-cli/references/recipes.md）
allowed-tools:
  - Read
  - Write
  - Bash
...
**必须遵守：**
- ✅ 使用 `gemini-cli chat --image` 进行图片分析
- ✅ 参考 `skills/gemini-cli/references/recipes.md` 中的配方
- ✅ 进行至少 5 轮 Gemini 分析
...
- **核心能力**: 编排 gemini-cli 多轮视觉分析 + 设计元素整合
- **依赖**: `gemini-cli` skill（参考其 recipes.md 获取 prompt 模板）
...
**使用 gemini-cli**（参考 recipes.md 配方 1）：
