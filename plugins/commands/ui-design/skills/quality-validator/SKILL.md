...
| MCP 工具              | 用途                                 | 触发条件        |
| --------------------- | ------------------------------------ | --------------- |
| `sequential-thinking` | 结构化质量验证策略，确保评分客观准确 | 🚨 每次执行必用 |
| `auggie-mcp`          | 语义检索代码结构                     | 分析生成代码时  |

## 执行流程

### Step 0: 结构化质量验证规划（sequential-thinking）

🚨 **必须首先使用 sequential-thinking 规划质量验证策略**

```
mcp__sequential-thinking__sequentialthinking({
  thought: "规划质量验证策略。需要：1) 加载检查目标 2) 分析代码结构 3) 检查代码质量 4) 验证设计还原度 5) 计算综合评分",
  thoughtNumber: 1,
  totalThoughts: 5,
...
1. **检查目标加载**：加载生成的代码文件和设计规格
2. **代码结构分析**：使用 auggie-mcp + LSP 分析组件结构和类型
3. **代码质量检查**：语法、未使用代码、命名、职责单一、复用性
4. **设计还原度验证**：颜色、字体、间距、响应式、完整性
5. **综合评分计算**：汇总评分并生成验证报告
