...
function printUsage(): void {
  process.stdout.write(
    "Usage: npx tsx scripts/run-tpd-gemini-cli.ts --role <role> --prompt <text> [--mode <analyze|prototype>] [--focus <text>] [--run_dir <dir>]\n",
  );
}

function fail(message: string, code: number): never {
  process.stderr.write(`[run-tpd-gemini-cli] ${message}\n`);
  process.exit(code);
}

function artifactName(role: string, mode: string | undefined, runDir: string): string {
  if (role === "implementer" && mode === "prototype") {
    return normalizeJoin(runDir, "prototype-gemini.diff");
  }
  if (role === "implementer" && mode === "analyze") {
    return normalizeJoin(runDir, "analysis-gemini.md");
  }
  return normalizeJoin(runDir, `gemini-${role}.md`);
}
...
function main(): void {
  ...
  const payload = {
    ...
    command: ["npx", "tsx", "scripts/invoke-gemini.ts"],
    command_args: {
      role,
      mode: mode ?? null,
      prompt,
